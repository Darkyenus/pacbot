...
/** Singleton responsible for scheduling tasks. Task's create linked list structure that is being constantly consumed by Tasks
 * themselves. Every Task can have one successor task. When active Task finishes, successor takes over. When there is no
 * successor, TaskProcessor waits for more actions from outside and resumes processing as soon as any task arrives. */
public final class TaskProcessor {
	...
	/** Will stop processing as soon as last task finishes. Will not then continue to its successors nor wait for more. */
	...
	/** Given task will be added to the end of queue. Then it will be executed asynchronously. Therefore, this method doesn't block.
	 *
	...
	/** Task contains logic for program. It has one optional successor Task that will be executed when this Task finishes. This goes
	 * down recursively. */
	public static class Task {
		...
		/** Given task will be forcefully added right after this task. So it will be this task's direct successor. Any previous
		 * successors will be appended to this successor. {@see appendTask(Task)} for how it will be done. */
		...
		/** Given task will be added as successor to this Task. If this task has no successor, it will be direct successor. If this
		 * task has successor, it will be appended with same logic to him.
		 *
		...
		/** Override this if your task runs for any measurable amount of time. Returns true if robot doesn't move during this task.
		...
		/** Sets direct successor, discarding original one (if there was any). So anything queued after this Task will be discarded
		 * by this. */
